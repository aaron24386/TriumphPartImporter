...
function get_cv_invoice_itembins(placement, itemid, alllocations, otherstores) {
  ...
  var path =
    'invoicing/newinvoice/cv_invoice_itembins.php?placement=' +
    placement +
    '&alllocations=' +
    alllocations +
    '&itemid=' +
      ...
      $(document).on('keydown', function (e) {
        ...
        switch (e.which || e.keyCode) {
          ...
          case 61: // plus
            ...
            if (temp == 'txt_qtytoadd') e.preventDefault(); //prevent typing + in text field

            if (InvoiceBinQty[addItemBinQtyIndex].value == '') InvoiceBinQty[addItemBinQtyIndex].value = 0;
          ...
          case 173: //minus
            ...
            //this preventDefault works globally on this field once active so we need to be sure we
            //only apply this if the focus is within one of our bin qty fields otherwise they can't type + or - anywhere in Lizzy
            if (temp == 'txt_qtytoadd') e.preventDefault(); //prevent zoom
          ...
          case 9: //tab
            ...
            if (!(addItemBinQtyIndex <= InvoiceBinQty.length - 1)) {
              ...
              try {
                document.getElementById('txt_pricetoadd').focus();
              } catch (err) {}
            }
            break;
          case 13: //enter key
            if (addItemBinQtyIndex >= 0) save_cv_invoice_itemlocate('frm_invoice_itemlocate', 1, 0, false, 'open');
            break;
          case 27: //escape key
...
function add_invoice_itembins(binid, increment) {
  ...
  document.getElementById('txt_qtytoadd' + binid).value =
    Number(document.getElementById('txt_qtytoadd' + binid).value) + increment;
}
