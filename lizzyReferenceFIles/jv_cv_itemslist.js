...
function get_cv_itemslist(comingfrom, placement, count, is_kit, itemnumberfieldname) {
  ...
  //09.14.2022 tam - 216555 adding ASIN
  var path =
    'inventory/cv_itemslist.php?kit=' +
    is_kit +
    '&placement=' +
    placement +
    '&count=' +
    count +
    '&page=' +
    document.getElementById('global_inv_list').value +
    '&vendor=' +
    vendor +
    '&supplier=' +
    supplier +
  ...
  if (comingfrom == 'AddToKit')
    //create kit
    path = path + '&kitID=' + kitID;
  else path = path + '&kitID=0';
...
function call_cv_itemslist() {
  ...
  if (globalGetItemListTrue == true) {
    ...
    get_cv_itemslist(
      comm_cv_itemslist.comingfrom,
      comm_cv_itemslist.placement,
      comm_cv_itemslist.count,
      comm_cv_itemslist.is_kit,
      comm_cv_itemslist.itemnumberfieldname,
    );
  }
}
...
function kit_add_item(itemid) {
  ...
  var path =
    'inventory/ce_additemkit.php?itemid=' +
    itemid +
    '&kitid=' +
    kitID +
    '&loc=' +
    document.getElementById('drpLocation').value;
...
function call_kit_add_item() {
  if (comm_cv_itemslist.response.substring(0, 5) == 'Error') setError(comm_cv_itemslist.response);
  else {
...
function call_save_cv_itemslist() {
  ...
  } else setError(comm_cv_itemslist.response);
}

//09.18.2017 tam - 98271 function for RawMaterial
function raw_add_item(itemid) {
  var path = 'inventory/ce_rawitems.php?itemid=' + itemid + '&masterid=' + document.getElementById('hid_itemid').value;
...
function call_raw_add_item() {
  if (comm_cv_itemslist.response.substring(0, 5) == 'Error') setError(comm_cv_itemslist.response);
    ...
    if (comm_cv_rawitems.placement == undefined) var placement = NextAvailOverlay();
    else var placement = comm_cv_rawitems.placement;
...
function addAliasToItem(aliasnumber) {
  ...
  if (pn != undefined) {
    var path =
      'inventory/ce_addaliasfrompo.php?itemnumber=' +
      pn +
      '&aliasnumber=' +
      aliasnumber +
      '&supplierid=' +
      document.getElementById('drp_supplier').value;
