...
function getVOIPSkype() {
  $.get('getVOIPSkype.php', function (data) {
    VOIP = data;
  });
}
...
function getLocation(position) {
  if (position.coords.latitude != null && position.coords.latitude != undefined) latitude = position.coords.latitude;
...
function swap_objects(formname, func, focused) {
  ...
  for (var i = 0; i < elem.length; i++) {
    ...
    if (elem[i].id == focused) {
      if (document.forms[formname].btn_edit.value == document.getElementById('global_lang_save').value) elem[i].focus();
    }
...
function check_todate(from, to) {
  if (Date.parse(from.value) > Date.parse(to.value)) to.value = from.value; //set the to field to have the from field's value.
}
...
function browserResize() {
  try {
    ...
    Global_bwidth = Math.round(document.body.clientWidth * 0.02);
  } catch (err) {}
  ...
  if (readCookie('ssi_menumodule') == 1) {
    //invoicing module ( rentals )
    //rental
  ...
  //we are in invoicing rentals. otherwise we enter here and all is like it has always been
    ...
    try {
      if (document.getElementById('onMonthlyCalendar').value == 'Open') get_cv_monthly_view('reg_cal_view', lastdate);
    } catch (err) {}

    try {
      if (document.getElementById('onDailyCalendar').value == 'Open') get_cv_daily_view('reg_cal_view', lastdate);
    } catch (err) {}
    ...
    try {
      ...
      if (document.getElementById('onWeeklyCalendar').value == 'Open') get_cv_weekly_view('reg_adv_dash', lastdate);
    } catch (err) {}
  }
...
function call_lizzy_get_video() {
  if (comm_lizzy_get_video.response.substring(0, 5) != 'Error') {
    ...
    scroll(0, 0);
  } else setError(comm_lizzy_get_video.response);
}
...
function stopwatchStart(region, recall) {
  ...
  if (!document.getElementById('hid_pause') || !document.getElementById(region)) return;
  ...
  if (document.getElementById('hid_pause').value == 0) {
    ...
    document.getElementById(region).innerHTML =
      temp + '<div style="margin-left:15px; display:inline-block;">**PAUSED**</div>';
  }
}
...
function stopwatchStop(region) {
  ...
  hour = 0;
  window.clearTimeout(CALLTIMER);
  if (document.getElementById(region)) document.getElementById(region).innerHTML = '';
}
...
function getMessageAlerts(timeit) {
  ...
  if (timeit) window.setTimeout('getMessageAlerts( true );', MESSAGEALERTTIMER);
...
function getUserMessages(userid) {
  if (userid == 0 && getUserMessagesRunning) return;
  ...
  } else getUserMessagesRunning = false;
}
...
function call_cv_UserMessages() {
  if (comm_cv_usermessages.response != '') {
    ...
    document.getElementById(comm_cv_usermessages.misc3).scrollTop = document.getElementById(
      comm_cv_usermessages.misc3,
    ).scrollHeight;
  ...
  if (comm_cv_usermessages.misc4 == userchat1) {
    ...
    } else if (userchat2 > 0) getUserMessages(userchat2);
    else if (userchat3 > 0) getUserMessages(userchat3);
  } else if (comm_cv_usermessages.misc4 == userchat2) {
    if (userchat3 == 0) {
      getUserMessagesRunning = false;
      window.setTimeout('getUserMessages( 0 );', 5000);
    } else if (userchat3 > 0) getUserMessages(userchat3);
...
function textbox_resize(pix, field_id) {
  ...
  if (new_height > 0) box.style.height = new_height + 'px';
...
function nizexPrint(region) {
  ...
  a.onload = a.setTimeout(function () {
    a.print();
  }, 1200);
}
...
function is_checkbox_checked(formname) {
  ...
  var tmpcount = 0;
  for (var i = 0; i < elem.length; i++) {
    if (elem[i].type == 'checkbox') if (elem[i].checked == true) tmpcount++;
  }
...
function LizzyPhone(phone, contactid) {
  ...
  try {
    document.getElementById('reg_quickadd').style.visibility = 'hidden';
  } catch (err) {}
  ...
  if (phone != '') {
    ...
    if (browser.match(/iphone|android/i) || VOIP == 0) window.location.href = 'tel:+1' + phone;

    if (VOIP == 1) {
      //for Skype
      if (browser.match(/chrome/i)) {
        window.location.href = 'callto:001' + phone;
      } else window.location.href = 'callto:+1' + phone;
    }

    if (VOIP == 2 && JavaEnabled == 1)
      // for Hard Phones
    ...
    if (VOIP == 3 || (VOIP == 2 && JavaEnabled == 0)) {
      // for Hard Phones using Go module
      $.getJSON('/java/LizzyDialer.php?phone=' + phone + '&json=1', function (data) {
        $.ajax({
          url: 'http://localhost:9090/' + data.host + '/' + data.user + '/' + data.passwd + '/' + phone,
          type: 'GET',
        });
      });
    }
...
function Lizzy_Field_Validation(fieldtype, fieldvalue, reformat, fieldid) {
  if (reformat == undefined) reformat = 0;
...
function call_lizzy_field_validation() {
  if (comm_lizzy_field_validation.response.substring(0, 5) != 'Error') {
    ...
    if (comm_lizzy_field_validation.misc != '' && comm_lizzy_field_validation.misc2 == 0) {
      if (comm_lizzy_field_validation.misc3 == 1) document.getElementById(comm_lizzy_field_validation.misc).value = '0';
      else if (comm_lizzy_field_validation.misc3 == 2)
        document.getElementById(comm_lizzy_field_validation.misc).value = '0.00';
      else document.getElementById(comm_lizzy_field_validation.misc).value = '';
...
function SetURLAsLink(message, message2, fieldid) {
  ...
  if (temp != 'http://www' && temp != '' && temp2 != '') insertAtCursor(document.getElementById(fieldid), web); //located in js_master
}
...
function ImagePathForTextarea(message, fieldid, maxWidth) {
  ...
  if (temp != 'http://www' && temp != '') insertAtCursor(document.getElementById(fieldid), web); //located in js_master
}
//################################################################################
//################################################################################
...
function getNumber(num) {
  ...
  var multiplier = 1;
  if (num.substring(0, 1) == '-') multiplier = -1;
...
function navigateLogCall(contactid, zipid, TopicID, placement) {
  ...
  if (TopicID == undefined) Cal_Call_TopicID = '';
  else Cal_Call_TopicID = TopicID;
...
function call_logfacetoface() {
  ...
  } else setError(comm_logfacetoface.response);
}
...
function DisplayProgressBar() {
  if (ProgressNeeded) document.getElementById('lizzy_progress').style.display = 'block';
}
...
function getQuickKeys() {
  ...
  $.getJSON('ce_getquickkeys.php', function (data) {
    $.each(data, function (key, value) {
      QuickKeys[key] = new Array(4);

      for (var i = 0; i < value.length; i++) QuickKeys[key][i] = value[i];
...
function BuildOverlay(controlclassname, placement, response, scrolltop) {
  ...
  try {
    ...
    else $(controlclassname).css('margin-top', scroll + 'px');
...
function CenterOverlay(placement) {
  ...
  if (table_width == undefined) {
    //maybe working with a div rather than a table
...
function drop(ev, target, callback, confirmation) {
  ...
  if (confirmation != undefined && confirmation != '') callback(source, target, confirmation);
  else callback(source, target);
}
/****************************************/
...
function attachmentUploadInit(callback, singlefile, frm_name) {
  ...
  doc.ondrop = function (event) {
    ...
    if (frm_name == undefined) frm_name = 'frm_upload';
...
function logoUploadInit(callback, singlefile, frm_name) {
  ...
  //07.25.2013 naj - added check to supress errors when the drag and drop area does not exist.
  if (logodoc == undefined || logodoc == null) return false;

  logodoc.ondragover = function () {
    return false;
  };
  logodoc.ondragend = function () {
    return false;
  };
  logodoc.ondrop = function (event) {
    ...
    //11.20.2012 naj - check to see if a form name was passed, if not default to frm_upload.
    if (frm_name == undefined) frm_name = 'frm_upload_logo';
...
function WebstoreCSSUploadInit(callback, singlefile, frm_name) {
  ...
  //07.25.2013 naj - added check to supress errors when the drag and drop area does not exist.
  if (cssdoc == undefined || cssdoc == null) return false;

  cssdoc.ondragover = function () {
    return false;
  };
  cssdoc.ondragend = function () {
    return false;
  };
...
function ClearContactGlobals() {
  ...
  try {
    globalMergeContact1 = 0;
  } catch (err) {}
  try {
    globalMergeContact2 = 0;
  } catch (err) {}
  try {
    MergeConFlag = 0;
  } catch (err) {}
  try {
    InvGeneralContactID = 0;
  } catch (err) {}
  try {
    globalInvoiceDefaultFlag = '';
  } catch (err) {}
  try {
    ConsignmentContact = '';
  } catch (err) {}
  try {
    InternalInvoiceContact = '';
  } catch (err) {}
  try {
    globalScanTagContactID = 0;
  } catch (err) {}
  try {
    LocateContactFlag = '';
  } catch (err) {}
  try {
    LinkContactFlag = 0;
  } catch (err) {} //once a contact is selected, this holds the contactid
  ...
  try {
    globalContactScanTag = '';
  } catch (err) {}
  try {
    MovePartsContactVoidInvoice = false;
  } catch (err) {}
  try {
    GlobalCallContactID = 0;
  } catch (err) {}
  try {
    MoveARContact_ARID = 0;
  } catch (err) {}
}
...
function printIframe(iframe) {
  ...
  if (browser.match(/chrome|safari/i)) var docready = framedoc.body.getElementsByTagName('embed');

  if (browser.match(/firefox/i)) var docready = framedoc.body.getElementById('outerContainer');
  ...
  if (docready.length == 0) {
    ...
    setTimeout(function () {
      if (frame.contentWindow == undefined) frame.print();
...
function printPDFDocument(url) {
  ...
  banner.innerHTML =
    '<span style="position:relative; top:10%;"><span class="bold">Use the PDF viewer\'s controls to print or save the PDF.</span><br>Click Here to Return To Lizzy.</span><input type="hidden" id="printpdf_overlay" value="' +
    temp +
    '">';
...
if (typeof Storage != undefined) {
  if (localStorage.ClientHostName != undefined) ClientHostName = localStorage.ClientHostName;
}
...
function InSalesDash() {
  if (readCookie('ssi_menumodule') == 12) {
    if (
      ...
      return true;
  } //if on home screen and sales manager dash menu
  else {
    if (readCookie('ssi_menumodule') == 8) {
      if (globalMenuID == 727) return true;
    }
  }
...
function setupMasonryDash(main_container, item_selector) {
  ...
  function filter() {
    ...
    grid.filter(function (item) {
      ...
      else isFilterMatch = !filterFieldClass ? true : (element.getAttribute(filterSect) || '') === filterFieldClass;
  ...
  function serializeLayout(grid) {
    ...
    $(itemElems).each(function (i, itemElem) {
      ...
      if (dashids == '') dashids = itemElem.getElement().getAttribute('id');
      else dashids = dashids + '|' + itemElem.getElement().getAttribute('id');
    });
    //SaveDashWidget() is located in common/js_cv_widget_list.js
    if (dashids != '') SaveDashWidget('', 0, 'sort', dashids);
  }
}
...
function NextAvailOverlay() {
  ...
  for (var i = 0; i <= 3; i++) {
    if (i == 0) {
      //video_overlay
      if (document.getElementById('video_overlay').innerHTML != '') overlay[0] = true;
    } else {
      if (document.getElementById('overlay' + (i + 1)).innerHTML != '') overlay[i] = true;
    }
  }
...
function call_lizzy_save_paginglimit() {
  ...
  } else setError(comm_lizzy_save_paginglimit.response);
}
...
function scrollToId(id) {
  ...
  if (head) height = head.offsetHeight + 10;
  else height = 135;

  if (x) {
    x.scrollIntoView();
    if (!(window.innerHeight + window.scrollY) >= document.body.offsetHeight) window.scrollBy(0, height * -1);
  } else return;
}
