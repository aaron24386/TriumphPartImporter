...
function get_cv_invoice_itemlocate(placement, jobid) {
    if (jobid == undefined)
        jobid = 0;
    var path = 'invoicing/newinvoice/cv_invoice_itemlocate.php?numon=' + numon + '&invoice_steps=' + invoice_steps + '&invoiceid=' + globalInvoiceID;
...
function CheckPartPricePercentLocate() {
    ...
    if (document.getElementById('hid_isRental')) {
        if (document.getElementById('hid_isRental').value == '1' && invoice_type == 17)
            return true;
    }
    ...
    if (document.getElementById('hid_percentnotify').value > 0) {
        ...
        if (isNaN(newprice)) //returns true if not a number
            newprice = 0;

        try { var discount = parseFloat(document.getElementById('txt_discounttoadd').value); } catch (err) { var discount = 0 }
        if (isNaN(discount))//returns true if not a number
            discount = 0;
...
function save_cv_invoice_itemlocate(formname, status, invoiceitemid, forceorder, after) {
    ...
    if (document.getElementById('hid_qoh') && document.getElementById('hid_minorderqty')) {
        ...
        if (minorderqty > 0) {
            ...
            if (qty_req > 0)
                qty_to_order = (qty_req - qoh);

            qty_to_order = qty_to_order + force_order;
        }
    }
    if (qty_to_order > 0 && qty_to_order < minorderqty)
        var temp = "confirm( 'The current qty to be ordered will be below the minimum qty to order set on the part. Min. order qty :" + minorderqty + "');";
    else
        var temp = true;

    //04.11.2019 jss - ( 147193 ) disable the add buttons to double clickers from getting i-d-10-t errors ;-)
    try { document.getElementById('btn_additem').disabled = true } catch (err) { };
    try { document.getElementById('btn_additemclose').disabled = true } catch (err) { };
    try { document.getElementById('btn_additemtop').disabled = true } catch (err) { };
    try { document.getElementById('btn_additemclosetop').disabled = true } catch (err) { };
    ...
    if (eval(temp)) {
        if (CheckPartPricePercentLocate()) {
            ...
            var path = 'invoicing/newinvoice/ce_invoice_itemlocate.php?status=' + status +
                '&forceorder=' + forceorder +
                '&invoiceitemid=' + invoiceitemid + '&invoiceid=' + globalInvoiceID +
                '&itemid=' + document.getElementById('hid_invoice_itemid').value + '&contactid=' +
                document.getElementById('global_contactid').value + '&invtype=' + invoice_type;
        ...
        //continue or they do not and we just show an error message
            ...
            if (document.getElementById('hid_can_continue').value == 'yes') {
                ...
                //so if you cancel out of the confirm window, I am making them active again
                    ...
                    try { document.getElementById('btn_additem').disabled = false } catch (err) { };
                    try { document.getElementById('btn_additemclose').disabled = false } catch (err) { };
                    try { document.getElementById('btn_additemtop').disabled = false } catch (err) { };
                    try { document.getElementById('btn_additemclosetop').disabled = false } catch (err) { };
                }
            }
            else {
    ...
    if (comm_cv_invoice_itemlocate.response.substring(0, 5) != 'Error') {
        ...
        if (comm_cv_invoice_itemlocate.misc5 == 'open') {
            if (document.getElementById('reg_alsosell').innerHTML != '') {
                ...
                try { document.getElementById("reg_manuf_sup").innerHTML = ''; }
                catch (err) { }
                ...
                get_cv_invoice_itemlocate(comm_cv_invoice_itemlocate.placement, jobid);
        }
        else
            close_overlay(comm_cv_invoice_itemlocate.placement);
...
function getNewInvoicePartInfo(isAlsoSell, ChildItemID) {
    ...
    var path = 'invoicing/newinvoice/cs_invoice_items.php?partnumber=' + encodeURIComponent(document.getElementById("txt_partnumbertoadd").value) + '&contactid=' + document.getElementById('global_contactid').value + '&itemid=' + itemid;
...
function call_cs_invoice_items() {
    ...
    if (comm_cs_invoice_items.response.substring(0, 5) != 'Error') {
        if (comm_cs_invoice_items.response.substring(0, 5) != 'blank') {
            ...
            try { document.getElementById('hid_qoh').value = myarray[16]; } catch (err) { }
            try { document.getElementById('hid_minorderqty').value = myarray[17]; } catch (err) { }
            //***********************

            try { document.getElementById('txt_discounttoadd').value = myarray[9]; } catch (err) { }

            if (myarray[4] == 1)
                document.getElementById('po_field').style.display = 'inline';
            else
                document.getElementById('po_field').style.display = 'none';
            ...
            if (myarray[13] == 1)
                document.getElementById("txt_pricetoadd").readOnly = true;
            else
                document.getElementById("txt_pricetoadd").readOnly = false;
        ...
        if (myarray) {
            ...
            if (!comm_cs_invoice_items.isAlsoSell)
                get_cv_invoice_alsosell('reg_alsosell', myarray[3]);
        }
    }
    else {
...
function itemlocate_AddPriceCodePickListInv() {
    ...
    try { var jobid = document.getElementById('drp_jobs').value; } catch (err) { var jobid = 0; }
    path += '&jobid=' + jobid;
...
function sendToLizzyARI(data) {
    ...
    try {
        if (document.getElementById('drp_jobs') != '')
    ...
    var path = 'invoicing/ce_partsmart.php?action=AddParts&invoiceid=' + globalInvoiceID +
        '&jobid=' + jobid +
